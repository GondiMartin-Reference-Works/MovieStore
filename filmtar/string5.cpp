...
#ifdef _MSC_VER
// MSC ne adjon figyelmezteto üzenetet a stringkezelo függvényekre.
  #define _CRT_SECURE_NO_WARNINGS
#endif

#include <iostream>             // Kiiratáshoz
...
/// Konstruktor: egy char karakterbol (createStrFromChar)
...
String::String(const char *p) {
    ...
    // Helyet foglalunk
    pData = new char[len+1];
    // Bemásoljuk a stringet, ami le van zárva 0-val igy használható az strcpy is
    strcpy(pData, p);
}
...
// indexhiba esetén dobjon egy const char * tipusú hibát!
...
String String::operator+(const String& rhs_s) const {
    ...
    // Felszabaditja a temp adattaerületét
    delete []temp.pData;
    // lefoglalja a memóriát az új stringnek.
    temp.pData = new char[temp.len+1];
    // Az elejére bemásolja az elso stringet
...
// << operator, ami kiir az ostream-re
std::ostream& operator<<(std::ostream& os, const String& s0) {
    os << s0.c_str();
    return os;
}

// << operátor, ami beolvas az istreamrol egy sort
std::istream& operator>>(std::istream& is, String& s0) {
	...
	is.setf(fl);						// visszaállitjuk a flag-eket
    return is;
}
